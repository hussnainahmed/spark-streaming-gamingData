...
//    case class GameData(cid: String, cname: String, email: String, gender: String, age: Int, address: String, country: String, register_date: String, friend_count: Int, lifetime: Int, game1: Int, game2: Int, game3: Int, game4: Int, revenue: Int, paid_customer: String)
    ...
    val Array(brokers, topics, maxRate, ratebalance) = args

    val sparkConf = new SparkConf().setAppName("Game-Data-Streaming-DK").set("spark.streaming.backpressure.enabled", ratebalance).set("spark.streaming.kafka.maxRatePerPartition", maxRate)
    ...
    //val zkQuorum = "cdh-node-5.novalocal,cdh-node-7.novalocal,cdh-node-9.novalocal"
    //val kafkaParams = Map[String, String]("metadata.broker.list" -> brokers)
    ...
    //val lines = KafkaUtils.createStream(ssc, zkQuorum, "-stream", topicMap).map(_._2)
